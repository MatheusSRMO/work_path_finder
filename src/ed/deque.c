...
#define INITIAL_MAP_SIZE 10e1 // numero par para facilitar a divisao
...
int fd(int tam, int front_map_idx, int back_map_idx) {
...
void __deque_centralize_map(Deque* d) {
    ...
    int delta = fd(tam, front_map_idx, back_map_idx);
    ...
    for (int i = front_map_idx; i < back_map_idx; i++) {
        if(new_buffer[new_front_map_idx] != NULL)
            __deque_free_bucket(new_buffer[new_front_map_idx]);
...
void __deque_realoc_map(Deque* d, int new_map_size) {
    ...
    if (deque_size(d) >= (d->map_size * BUFFER_SIZE) * 0.8) {
